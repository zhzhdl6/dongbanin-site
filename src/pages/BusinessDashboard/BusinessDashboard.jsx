...
import { IconChart, IconRefresh, IconLogout, IconSearch, IconChevronDown, IconStar } from '../../components/Icon';
...
const SortArrow = ({ active, dir }) => (
  <span className={styles.sortArrow} style={{ display: 'inline-flex', alignItems: 'center' }}>
    {active ? (dir === 'asc' ? '▲' : '▼') : <IconChevronDown width={10} height={10} />}
  </span>
);
    ...
    <div className={styles.dashboardContainer}>
      ...
      <div className={styles.dashboardHeader}>
        <div className={styles.headerLeft}>
          <h2 className={styles.dashboardTitle}><IconChart width={22} height={22} style={{ verticalAlign: 'middle' }} /> 사업자 대시보드</h2>
          <p className={styles.dashboardSubtitle}>
            {companyName ? `${companyName} ` : ''}사업자 전용 경쟁사 분석 리포트
          </p>
        </div>
        <div className={styles.headerRight}>
          <button className={styles.refreshBtn} onClick={handleRefresh}><IconRefresh width={14} height={14} /> 새로고침</button>
          <button className={styles.logoutBtn} onClick={onLogout}><IconLogout width={14} height={14} /> 로그아웃</button>
        </div>
      </div>
      ...
      <div className={styles.statsGrid}>
        ...
        <div className={styles.statCard}>
          ...
          <span className={styles.statValue}><IconStar width={18} height={18} style={{ verticalAlign: 'middle', color: '#F6AD55' }} /> {stats.avgRating}</span>
        </div>
        <div className={styles.statCard}>
      ...
      <div className={styles.toolbar}>
        <div className={styles.searchBox}>
          <span className={styles.searchIcon}><IconSearch width={16} height={16} /></span>
          <input
            type="text"
            ...
            {pageRows.length === 0 ? (
              ...
              pageRows.map(row => (
                <tr key={row.id}>
                  ...
                  <td><span className={styles.ratingText}><IconStar width={12} height={12} style={{ verticalAlign: 'middle' }} /> {row.rating}</span></td>
