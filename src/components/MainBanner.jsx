...
import { IconGift, IconDog, IconTree, IconPlay, IconPause } from './Icon';

function MainBanner() {
  const originalBanners = [
    { id: 1, badge: 'OPEN EVENT', title: '동반인 첫 가입시\n전체 미용/식당 10% 쿠폰팩', Icon: IconGift, bg: 'linear-gradient(135deg, #FF8A3D 0%, #FFB074 100%)' },
    { id: 2, badge: '쇼핑 추천', title: '쿠팡 인기 반려용품\n최저가 라인업 보러가기', Icon: IconDog, bg: 'linear-gradient(135deg, #3B82F6 0%, #86B1F9 100%)' },
    { id: 3, badge: '산책 가이드', title: '이번 주말 아이와 함께\n달리기 좋은 운동장 BEST 5', Icon: IconTree, bg: 'linear-gradient(135deg, #2FBB86 0%, #76DCB6 100%)' }
  ];
        ...
        {bannerList.map((banner, index) => (
          <SwiperSlide key={index} className={styles.swiperSlideCustom}>
            <div className={styles.bannerWrapper} style={{ background: banner.bg }}>
              ...
              <div className={styles.bannerGraphic}><banner.Icon width={64} height={64} /></div>
            </div>
          </SwiperSlide>
        ))}
      </Swiper>
